...
function shortcode_testimonials($atts, $content = null, $code) {
	...
	$output .= '<div class="testimonials-container"><div class="testimonials-inner">';		
	if($type == "slider") { 
		...
		$output .= '<div class="item'.$class.'"><div class="product-block">';
			$output .= '<div class="single-testimonial">';			
				$output .= '<div class="testimonial-content">';
					$output .= '<div class="testimonial-top"><blockquote><q>'.substr($contents, 0, 180).'</q></blockquote></div>';
...
function shortcode_button($atts, $content = null) {
   extract(shortcode_atts(array(
	...
	$output = '';
	$output .= '<div class="button_content_inner '.$classname.'">';
	if(!empty($icon)){
		if($icon_align == 'left')
...
function shortcode_logo($atts, $content = null) {
	...
	$output .= '<div id="brand-products" class="tm_logocontent animated '.$align.'" data-animated="fadeInUp">';
	if($type == 'slider'):
...
function shortcode_banner($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		$output = '';
		$output .='<div class="tm_banner column '.$class.'">';
...
function shortcode_single_slide($atts, $content = null, $code) {
	...
	$output = '';
	$output .= '<div class="banner-slider-container animated" data-animated="fadeInRight">';
	$output .= '<div class="slider-container-inner">';	
	$output .= '<div class="title">'.$title.'</div>';
	$output .= '<div class="home-slider-inner slider-loading"></div><div class="flexslider_'. $class .'">';
...
function shortcode_portfolio($atts, $content = null, $code) {
	...
	$output .= '<ul class="portfolio_'.$column.'column">';
...
$output .= '</div></div>';
		$output .= '</div>';
		$output .= '<div class="desc"><h5><a href="'.get_permalink().'">'.get_the_title().'</a></h5>';
		$output .=  '<div class="contents">'.templatemela_portfolio_excerpt($excerpt_length).'</div></div>';
		$output .= '</li>';
		$i++;
	}
	$output .= '</ul>';
...
function shortcode_container($atts, $content = null){
	...
	$variables .= 'margin:'.$margin.';';
		...
		if(!empty($background_image)):
			...
			if($parallax == true):
				if(!empty($background_image)):
					$datasource = 'data-source-url='.$background_image;
				endif;
			$classname .= ' parallax ';
		endif;
	...
	$output = '';
	$output .= '<div class="main-container '.$align.' '.$classname.'" style="'.$variables.';" '.$datasource.'>';
...
/*============ Container ===========*/
function shortcode_sub_container($atts, $content = null){
	extract(shortcode_atts(array(
		'padding' => '0',
		'margin' => '0 -15px',
		'align'=> '',
	...
	$variables .= 'overflow: hidden;';
	$output = '';
	$output .= '<div class="sub-main-container '.$align.' '.$classname.'" style="'.$variables.'">';
	$output .= '<div class="sub-inner-container">';	
	$output .=	do_shortcode($content);
	$output .= '</div></div>';
	return $output;
}
add_shortcode('sub_container', 'shortcode_sub_container');
...
function shortcode_static_text($atts, $content = null){
	...
	$output = '';
	$output .= '<div class="static-text-container '.$align.' '.$class.'">';
...
/***************** Static Text ****************/
function shortcode_static_text_block($atts, $content = null){
	extract(shortcode_atts(array(
	'align' => 'left',
	'class' => '',
	'text1' => '',
	...
	$output = '';
	$output .= '<div class="static-text-container '.$align.' '.$class.'">';
	$output .= '<div class="text_block"><span class="text1">'.$text1.'</span><span class="text2">'.$text2 .'</span></div>';
	$output .= '</div>';	
	return $output;
}
add_shortcode('text_block', 'shortcode_static_text_block');
/***************** Blog Posts ****************/
function shortcode_blog_posts_container($atts, $content = null, $code) {
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		$src = mr_image_resize($image, 380, 250, true, 't', false);
		if( empty ( $src ) || $src == 'image_not_specified' ):
			$src = get_template_directory_uri()."/images/megnor/placeholder.png";
			$src = mr_image_resize($src, 380, 250, true, 't', false);			
		endif;
			...
			$output .= '<div class="container-inner">';
				$output .= '<div class="post-image">';
					...
					$output .= '<a href="'.$image.'" data-lightbox="example-set" class="icon"></a>';	
				$output .= '</div>';
				...
				if($comments->total_comments > 0):
					$output .= '<div class="comments-link"><a href="'.get_permalink().'"><i class="fa fa-comment"></i>'.$text.'</a><div class="post-author1"><i class="fa fa-user"></i>'.get_the_author().'</div></div>';
				else:
					$output .= '<div class="comments-link"><a href="'.get_permalink().'"><i class="fa fa-comment"></i>'.$text.'</a><div class="post-author1"><i class="fa fa-user"></i>'.get_the_author().'</div></div>';
				endif;
				...
				$output .= '<div class="post-description">'.templatemela_blog_post_excerpt(15).'</div>';
				$output .= '</div></div></div>';
		$i++;
	endwhile;
	wp_reset_postdata();
...
function shortcode_newsletter_text($atts, $content = null){
  ...
  $output .= '<div class="newslettercontainer animated" data-animated="fadeInLeft">';

  $output .= '<div class="newslettercontainerinner">';

  $output .= '<div class="news-icon"></div><div class="news-top">';

  if(!empty($text1))

    $output .= '<h1 class="news-title">'.$text1.'</h1>';

  if(!empty($text2))

    $output .= '<div class="news-text">'.$text2.'</div></div>';
...
function shortcode_woo_products_container($atts, $content = null, $code) {
	...
	$logotype = $type;
	$output = '';
	$output .= '<div class="woo-content products_block animated '.$product.'" data-animated="fadeInLeft" >';
	
	if($type == "slider") { 
		$output .= '<div id="'.$items_per_row.'_woo_carousel" class="woo-carousel">';
	} else {
		$output .= '<div id="woo-grid" class="woo-grid woo_products cols-'.$items_per_row.'">';
	}
	$output .=	do_shortcode($content).'</div></div>';
...
function shortcode_cmsbanner($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		if($link_url != '#')
		{
			$output .= '<a href='.$link_url.' class="cms-content" target="_blank">';
		}
		else{
			$output .= '<a href='.$link_url.' class="cms-content">';
		}
		$output .= '<span class="cms-content-inner"><span class="text1">'.$text1.'</span><span class="text2">'.$text2.'</span></span>';	
		$output .= '</a>';
		$output .= '</div>';
		return $output;
}
...
function shortcode_woo_sub_categories($atts, $content = null) {
   extract(shortcode_atts(array(
     	'category_ids' => '110,111,131',
		'static_text1' => 'Womens Lookbook Maison',
		'static_text2' => 'Winter 2014 season'
	...
	$output .= '<div class="item-block"><div class="left-block"><ul class="da-thumbs">';
	$i = 1;
	...
	$output .= '<a href="'.get_category_link( $category_ids ).'"><img src="'.$image.'" title="'.$category_ids->name.'" alt="'.$category_ids->name.'"/><span class="cat-name"><span class="name">'.$category_ids->name.'</span><span class="btn">Shop Now</span></span></a></li>';
		}
		$i++;
	}
	$output .= '</ul></div>';
	$output .= '<div class="right-block">';
	$output .= '<div class="category"><div class="cat-desc"><a href="'.get_category_link( $category_ids ).'">'.$static_text2.'</a></div>';
	$output .= '<div class="cat-title"><a href="'.get_category_link( $category_ids ).'">'.$static_text1.'</a></div></div>';
	$output .= '<div class="cat-image"><a href="'.get_category_link( $category_ids ).'"><img src="'.$image.'" title="'.$category_ids->name.'" alt="'.$category_ids->name.'"/></a></div>';
	$output .= '</div></div>';
	$output .= '</div>';
	return $output;
}
...
function shortcode_product_tabs($atts, $content = null)
{
	...
	$output .= '<div id="horizontalTab">';
		$output .= '<ul class="resp-tabs-list">';
			$output .= '<li class="hb-animate-element top-to-bottom featured">'.$tab1_text. '<div class="tab-title"></div></li>';
			$output .= '<li class="hb-animate-element top-to-bottom latest">'.$tab2_text. '<div class="tab-title"></div> </li>';
			$output .= '<li class="hb-animate-element top-to-bottom bestseller">'.$tab3_text. '<div class="tab-title"></div></li>';	
		$output .= '</ul>';
	...
	$output .= '<div id="categorytab">';
		...
		$output .= '<div class="resp-tabs-container">';
			foreach($category_ids_array as $key){
					...
					$output .= do_shortcode('[woo_products type="'.$type.'" items_per_row="'.$items_per_row.'"][product_category  per_page="'.$items_per_page.'" category="'.$term_category_slug.'"][/woo_products]');
			}
		$output .= '</div>';
	$output .= '</div>';
	return $output;
...
function tm_mapshortcode( $atts, $content = null )
{
	...
	if($title){
		$output .= '<div class="map-title"><i class="fa fa-arrow-left"> </i>'.$title.'</div>';
	}
	$output .= '
